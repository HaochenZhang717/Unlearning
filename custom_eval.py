...
import difflib
from peft import PeftModel  # 确保开头 import 了这个
...
def main():
    ...
    if args.model_id.startswith("llava"):
        ...
        if os.path.exists(os.path.join(args.cache_path, "adapter_config.json")):
            print(f"Loading LoRA adapters from: {args.cache_path}")
            model = PeftModel.from_pretrained(model, args.cache_path)
            model = model.merge_and_unload()
        else:
            raise ValueError("Warning: No adapter found in cache_path, using base model only.")
